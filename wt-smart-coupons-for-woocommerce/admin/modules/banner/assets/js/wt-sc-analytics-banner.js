jQuery(document).ready(function($) {
    ...
    jQuery(document).on('click', '.wt_sc_analytics_banner_dismiss', function(e) {
        e.preventDefault();
   
        const $banner = jQuery(this).closest('.wt_sc_analytics_banner');
        const $button = jQuery(this);
        $banner.css('opacity', '0.5');
        $button.prop('disabled', true); /** Disable button to prevent multiple clicks  */
        
        $.ajax({
            url: wt_sc_analytics_banner_params.ajaxurl,
            type: 'POST',
            data: {
                action: 'wt_sc_dismiss_analytics_banner',
                nonce: wt_sc_analytics_banner_params.nonce
            },
    ...
    function checkPathAndToggleBanner() {
        ...
        const $banner = $('.wt_sc_analytics_banner');
